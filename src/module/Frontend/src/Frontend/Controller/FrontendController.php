	...
	public function ajaxcommentAction(){
		...
		if($request->isPost()){
			...
			if(!$new_comment_id = $this->getCommentTable()->saveComment($comment)){
				...
				$response->setContent(\Zend\Json\Json::encode(array('response' => true, 'comment_time' => $comment->post_time)));
			}
		}
		return $response;
	}
	...
	*/
	...
	public function ajaxmailAction(){
		...
		if($request->isPost()){
			...
			$mail->setBody($body);
			$mail->setFrom('info@244paddington.com.au', '244paddington.com.au');
