...
class SectionController extends AbstractActionController
{
	...
	public function noticeAction(){
		return new ViewModel(array(
			"notice_text" => $this->getSectionTable()->getSectionByName("notice"),
		));
	}
	
	public function editnoticeAction(){
		$id = 7;
		$section = $this->getSectionTable()->getSectionByName("notice");
		...
		if ($request->isPost()) {
			...
			if ($form->isValid()) {
				$this->getSectionTable()->saveSection($form->getData());
				return $this->redirect()->toRoute('section',array('action'=>'notice'));
			}
		}
